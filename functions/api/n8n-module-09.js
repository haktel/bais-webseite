const N8N_MODULE_09_WEBHOOK="https://6wejmb5u.rpcld.co/webhook/bais-academy-modul-09";
...
export async function onRequestPost({request}){
 ...
 if(!["grounded","unknown","injection","cost"].includes(scenario))return Response.json({ok:false,error:"Invalid lab scenario"},{status:422});
 try{
  const upstream=await fetch(N8N_MODULE_09_WEBHOOK,{method:"POST",headers:{"Content-Type":"application/json","Accept":"application/json","User-Agent":"BAIS-Academy/1.0"},body:JSON.stringify({scenario})});
 ...
 }catch{return Response.json({ok:false,error:"Live RAG lab unavailable"},{status:502,headers:{"Cache-Control":"no-store"}});}
}
