...
import{assertSameOrigin,createSession,ensureAuthSchema,findCustomerEmailVerification}from"../../../_lib/auth.js";
...
export const onRequestPost=async context=>{
 ...
 try{
  ...
  if(verification.expires_at<=now&&!verification.verified_at)throw new ApiError(410,"verification_expired","Bestätigungslink ist abgelaufen. Bitte fordern Sie eine neue E-Mail an.");
  const alreadyVerified=Boolean(verification.verified_at)&&verification.status==="active";
  if(!alreadyVerified){
   await db.batch([
    db.prepare("UPDATE users SET status='active' WHERE id=? AND role='customer'").bind(verification.user_id),
    db.prepare("UPDATE customer_email_verifications SET verified_at=? WHERE user_id=? AND token_hash=?").bind(now,verification.user_id,verification.tokenHash),
  ...
  return json({ok:true,alreadyVerified,user:{displayName:verification.display_name,email:verification.email,role:"customer"},commercial:{customerNumber:customer?.customer_number||null},message:alreadyVerified?"E-Mail-Adresse war bereits bestätigt.":"E-Mail-Adresse bestätigt. Ihr Kundenkonto ist jetzt aktiv.",requestId:traceId},200,{"set-cookie":session.cookie});
