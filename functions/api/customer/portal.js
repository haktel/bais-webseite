...
export const onRequestGet=async({request,env})=>{
 ...
 try{
  ...
  const [organization,projects,milestones,documents,approvals]=await Promise.all([
   ...
   db.prepare("SELECT d.id,d.project_id,d.name,d.version,d.created_at FROM documents d JOIN projects p ON p.id=d.project_id WHERE p.organization_id=? ORDER BY d.project_id,d.created_at DESC").bind(org).all(),
