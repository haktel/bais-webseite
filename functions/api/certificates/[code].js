...
export const onRequestGet=async({request,env,params})=>{
 ...
 try{
  ...
  return json({ok:true,certificate:{code:row.public_code,holder:row.display_name,title:row.title,course:row.course_title,issuedAt:row.issued_at,status:row.revoked_at?"revoked":"valid"},requestId:id},200,{"cache-control":"public, max-age=300"});
