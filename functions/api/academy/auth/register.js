...
export const onRequestPost=async({request,env})=>{
 ...
 try{
  ...
  const now=new Date().toISOString(),userId=crypto.randomUUID(),courseId="course-"+slug,runId="run-"+slug+"-self-paced",requestEntryId=crypto.randomUUID(),credential=await hashPassword(password);
  await db.batch([
   ...
   db.prepare("INSERT INTO audit_events(id,actor_user_id,event_type,entity_type,entity_id,metadata_json,created_at) VALUES(?,?,?,?,?,?,?)").bind(crypto.randomUUID(),userId,"academy.account.created","user",userId,JSON.stringify({courseSlug:slug,access:"pending",enrollmentRequestId:requestEntryId}),now)
  ]);
  ...
  return json({
   ...
   access:{courseSlug:slug,status:"pending"},
   message:"Ihr Academy-Konto wurde erstellt. Der Kurszugang wird nach Freigabe aktiviert.",
   requestId:traceId
