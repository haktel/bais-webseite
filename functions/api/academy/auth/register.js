...
export const onRequestPost=async({request,env})=>{
 ...
 try{
  ...
  const body=await readJson(request),displayName=cleanText(body.displayName,120),email=normalizeEmail(body.email),password=body.password,slug=cleanText(body.courseSlug,120),title=academyProgram(slug);
  ...
  const now=new Date().toISOString(),userId=crypto.randomUUID(),courseId="course-"+slug,runId="run-"+slug+"-self-paced",credential=await hashPassword(password);
  const preApproved=await db.prepare("SELECT id FROM enrollment_requests WHERE email=? AND course_id=? AND status='approved' ORDER BY created_at DESC LIMIT 1").bind(email,courseId).first();
  const statements=[
  ...
  if(preApproved){
   statements.push(
    db.prepare("INSERT OR IGNORE INTO enrollments(id,user_id,course_run_id,status,enrolled_at) VALUES(?,?,?,?,?)").bind(crypto.randomUUID(),userId,runId,"active",now),
    db.prepare("INSERT OR IGNORE INTO course_progress(user_id,course_id,progress_percent,status,updated_at) VALUES(?,?,?,?,?)").bind(userId,courseId,0,"not_started",now)
   );
   accessStatus="active";
  }else{
   requestEntryId=crypto.randomUUID();
   statements.push(
    db.prepare("INSERT INTO enrollment_requests(id,course_id,name,email,company,note,status,created_at) VALUES(?,?,?,?,?,?,?,?)").bind(requestEntryId,courseId,displayName,email,null,"Academy-Konto erstellt · Kurszugang ausstehend","new",now)
   );
  }

  statements.push(
   db.prepare("INSERT INTO audit_events(id,actor_user_id,event_type,entity_type,entity_id,metadata_json,created_at) VALUES(?,?,?,?,?,?,?)")
    .bind(crypto.randomUUID(),userId,"academy.account.created","user",userId,JSON.stringify({courseSlug:slug,access:accessStatus,enrollmentRequestId:requestEntryId}),now)
  );
  ...
  return json({
   ...
   message:accessStatus==="active"?"Ihr Academy-Konto wurde erstellt. Der freigegebene Kurszugang ist aktiv.":"Ihr Academy-Konto wurde erstellt. Der Kurszugang wird nach Freigabe aktiviert.",
   requestId:traceId
