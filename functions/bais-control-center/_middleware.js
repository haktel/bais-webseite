...
const continuePath=request=>{
 const path=new URL(request.url).pathname;
 return path.startsWith("/bais-control-center/")?path:"/bais-control-center/";
};
...
export const onRequest=async context=>{
 try{
  ...
  if(error?.status===401||error?.code==="mfa_setup_required"){
   ...
   target.searchParams.set("continue",continuePath(context.request));
