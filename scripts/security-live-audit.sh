...
if [ "$code" = "400" ] && grep -qi '^x-bais-customer-register: identity-only-v2' /tmp/headers; then pass "customer registration identity-only v2 is live and Turnstile protected"; else fail "customer registration v2 expected 400 + build marker, got HTTP ${code}"; fi
