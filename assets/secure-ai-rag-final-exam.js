(()=> {
 ...
 const api=async(options={})=>{const r=await fetch("/api/academy/secure-ai-rag-final-exam",{credentials:"same-origin",headers:{"Accept":"application/json","Content-Type":"application/json"},...options});const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.error?.message||"Anfrage fehlgeschlagen.");return d;};
