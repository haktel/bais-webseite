(()=> {
  const STORAGE={
    last:"bais-kih-m05-last",
    weak:"bais-kih-m05-weak",
    attempt:"bais-kih-m05-attempt"
  };
  ...
  const BANK=[
    q("KIH501","warnsignale","Welches der folgenden Muster gilt laut Modul als Warnsignal bei einer KI-Ausgabe?","Eine ungewöhnlich glatte, sehr sichere Formulierung zu einem eigentlich unsicheren Sachverhalt",["Eine kurze Antwortzeit der KI","Eine Antwort in einfacher Sprache","Eine Antwort mit mehreren Absätzen"],"Eine überzeugend klingende Formulierung zu einem unsicheren Thema ist ein typisches Warnsignal, das genauer geprüft werden sollte."),
    q("KIH502","dokumentation","Welche drei Angaben sollten laut Modul bei der Dokumentation einer Auffälligkeit mindestens enthalten sein?","Was aufgefallen ist, wann es aufgefallen ist und welche Situation zugrunde lag",["Nur der Name der verwendeten KI-Software","Nur das Datum, ohne weitere Angaben","Nur eine grobe Einschätzung, ohne konkrete Details"],"Was, wann und in welcher Situation sind die drei zentralen Angaben für eine nachvollziehbare Meldung."),
    q("KIH503","meldeweg","Wie wird der Meldeweg für KI-Auffälligkeiten in diesem Modul eingeordnet?","Als Teil der Qualitätssicherung, nicht als reine Bürokratie",["Als optionaler Schritt ohne praktischen Nutzen","Als Ersatz für die fachliche Prüfung von KI-Ausgaben","Als ausschließliche Aufgabe der IT-Abteilung"],"Der Meldeweg trägt aktiv zur Qualitätssicherung bei, indem wiederkehrende Probleme sichtbar werden."),
    q("KIH504","fehlerkultur","Was fördert laut Modul eine gesunde Fehlerkultur im Umgang mit KI?","Melden von Auffälligkeiten ohne Angst vor Schuldzuweisung",["Auffälligkeiten möglichst nicht zu erwähnen","Nur schwerwiegende Fehler zu dokumentieren","Meldungen ausschließlich anonym und ohne jede Nachverfolgung zuzulassen"],"Offenes Melden ohne Angst vor Schuldzuweisung verbessert die Qualität über die Zeit deutlich."),
    q("KIH505","quelle","Welches Warnsignal betrifft fehlende Nachvollziehbarkeit einer KI-Aussage?","Eine Zahl oder Aussage ohne erkennbare Grundlage oder Quelle",["Eine Aussage, die auf eine interne Leitlinie verweist","Eine Aussage mit Datum und Kontextangabe","Eine kurze, klar formulierte Antwort"],"Eine Aussage ohne erkennbare Grundlage ist ein klassisches Warnsignal, das besondere Prüfung verdient."),
    q("KIH506","lernen","Wozu dient das systematische Melden von Auffälligkeiten laut Modul langfristig?","Um wiederkehrende Probleme im KI-Einsatz systematisch zu erkennen und zu vermeiden",["Um einzelne Mitarbeitende für Fehler verantwortlich zu machen","Um die Nutzung von KI-Tools grundsätzlich einzuschränken","Um die Meldung an eine externe Behörde weiterzuleiten"],"Ziel ist die kontinuierliche Verbesserung des KI-Einsatzes, nicht die Zuweisung individueller Schuld."),
  ];
