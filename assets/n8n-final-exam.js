(()=> {
 ...
 const api=async(options={})=>{const r=await fetch("/api/academy/n8n-final-exam",{credentials:"same-origin",headers:{"Accept":"application/json","Content-Type":"application/json"},...options});const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.error?.message||"Anfrage fehlgeschlagen.");return d;};
 ...
 function renderStatus(d){
  ...
  status.innerHTML=d.eligible?"<strong>✓ Prüfungsfreigabe aktiv</strong><p>Alle 12 Module sind serverseitig mit 100% dokumentiert.</p>":"<strong>Noch nicht freigeschaltet</strong><p>Alle 12 Module müssen 100% erreichen. Fehlende Module sind unten markiert.</p>";
