(()=> {
  const STORAGE={
    last:"bais-kih-m03-last",
    weak:"bais-kih-m03-weak",
    attempt:"bais-kih-m03-attempt"
  };
  ...
  const BANK=[
    q("KIH301","art9","Was regelt Art. 9 DSGVO im Zusammenhang mit Gesundheitsdaten?","Er stellt Gesundheitsdaten als besondere Kategorie unter einen strengeren Schutz",["Er verbietet jede Verarbeitung von Gesundheitsdaten vollständig","Er gilt nur für staatliche Krankenhäuser","Er ersetzt alle anderen Datenschutzregeln der DSGVO"],"Art. 9 DSGVO ordnet Gesundheitsdaten als besondere Kategorie personenbezogener Daten ein, die einen strengeren Schutz benötigt."),
    q("KIH302","toolwahl","Was gilt laut Modul für echte, identifizierbare Patientendaten?","Sie gehören nur in ein von der Einrichtung freigegebenes System",["Sie dürfen in jedes gut bewertete öffentliche KI-Tool eingegeben werden","Sie dürfen nur mündlich weitergegeben werden","Sie unterliegen keinen besonderen Anforderungen an das Tool"],"Öffentliche, nicht freigegebene Tools sind für echte Patientendaten in aller Regel nicht geeignet."),
    q("KIH303","risikoquelle","Was ist laut Modul die häufigste Ursache für Datenschutzvorfälle im KI-Kontext?","Die einfache Eingabe echter Patientendaten in ein nicht dafür vorgesehenes, öffentliches Tool",["Komplexe, gezielte Cyberangriffe auf KI-Anbieter","Fehler in der KI-Modellarchitektur selbst","Zu lange Wartezeiten bei der Dateneingabe"],"Die meisten Vorfälle entstehen durch die Eingabe echter Daten in ungeeignete Tools, nicht durch komplexe Angriffe."),
    q("KIH304","minimierung","Was bedeutet Datensparsamkeit im Umgang mit KI-Tools laut diesem Modul?","Nur so viele Daten einzugeben, wie für die konkrete Aufgabe tatsächlich nötig sind",["Grundsätzlich alle verfügbaren Daten einzugeben, um bessere Ergebnisse zu erhalten","Daten nur einmal jährlich zu verarbeiten","Auf jede Dokumentation vollständig zu verzichten"],"Datensparsamkeit bedeutet, nur die für die Aufgabe notwendigen Daten zu verwenden und identifizierende Angaben wegzulassen, wenn sie nicht gebraucht werden."),
    q("KIH305","unsicherheit","Was sollte laut Modul geschehen, wenn unklar ist, ob ein Tool für Gesundheitsdaten freigegeben ist?","Vor der Nutzung bei der zuständigen Stelle in der Einrichtung nachfragen",["Das Tool einfach ausprobieren und die Ergebnisse bewerten","Die Daten sicherheitshalber doppelt eingeben","Die Entscheidung der KI selbst überlassen"],"Im Zweifel gilt: vor der Nutzung klären, statt eigenmächtig zu entscheiden."),
    q("KIH306","beispiel","Welches Beispiel für Datensparsamkeit nennt das Modul?","Einen Beispielfall ohne echten Namen und ohne genaues Geburtsdatum zu formulieren, wenn nur eine allgemeine Formulierungshilfe benötigt wird",["Immer den vollständigen echten Patientennamen zu verwenden","Datensparsamkeit ist laut Modul in der Praxis nicht umsetzbar","Nur die Krankenkasse darf über Datensparsamkeit entscheiden"],"Wenn eine Aufgabe keine echten Identifikationsdaten erfordert, sollten diese testweise weggelassen werden."),
  ];
