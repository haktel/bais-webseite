...
test("R2 customer documents are tenant-scoped, type-limited and finalized before visibility",()=>{
 ...
 const nativeUpload=read("functions/api/customer/documents/upload.js");
 const nativeFile=read("functions/api/customer/documents/file.js");
 ...
 assert.match(helper,/onlyIf:\{etagMatches/);
 ...
 assert.match(download,/r2StorageMode/);
 assert.match(nativeUpload,/assertSameOrigin/);
 assert.match(nativeUpload,/hasCustomerContentAccess/);
 assert.match(nativeUpload,/TransformStream/);
 assert.match(nativeFile,/expectedPrefix/);
 assert.match(nativeFile,/hasCustomerContentAccess/);
 assert.match(nativeFile,/cache-control/);
