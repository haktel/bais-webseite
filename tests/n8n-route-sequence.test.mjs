  ...
  return{
    prepare(sql){
      return{
        bind:(...args)=>({
          first:async()=>{
            if(sql.startsWith("SELECT s.id AS session_id"))return{session_id:"s1",user_id:"u1",expires_at:future,display_name:"Learner",email:"learner@example.com",role:"student",status:"active"};
